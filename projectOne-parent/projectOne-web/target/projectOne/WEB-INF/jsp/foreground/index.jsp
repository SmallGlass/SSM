		...
		<div class="span9">
			<div>
				<DIV style="width: 330px; height: 228px;" class="tuhuo">
					...
					<c:forEach var="imageNews" items="${imageNewsList}">
						<A href="newsDetail?newsId=${imageNews.newsId }">
							<IMG style="display: none;" class="tu_img" src="../images/${imageNews.imageName }" width="330" height="208" />
						</A>
					</c:forEach>
					<c:forEach var="imageNews" items="${imageNewsList}">
						<P style="height: 20px;" class="tc"><A href="newsDetail?newsId=${imageNews.newsId }" target="_blank" title="${imageNews.title }">${fn:substring(imageNews.title,0,18) }</A></P>
					</c:forEach>
			...
			<div class="newsHeader_list">
				<div class="newsHeader">
					<h3><a href="newsDetail?newsId=${headNews.newsId }" title="${headNews.title }">${fn:substring(headNews.title,0,10) }</a></h3>
					<p>${fn:substring(headNews.content,0,40) }...<a href="newsDetail?newsId=${headNews.newsId }" >[查看全文]</a></p>
				</div>
					...
					<div class="currentUpdateDatas">
						<table width="100%">
							<c:forEach var="newestNews" items="${newestNewsList }" varStatus="status">
								...
								<td width="50%">
									<a href="newsDetail?newsId=${newestNews.newsId }" title="${newestNews.title }">${fn:substring(newestNews.title,0,12) }</a>
								</td>
				...
				<div class="datas">
					<ul>
						<c:forEach var="hotSpotNews" items="${hotSpotNewsList }">
							<li><a href="newsDetail?newsId=${hotSpotNews.newsId }" title="${hotSpotNews.title }">${fn:substring(hotSpotNews.title,0,15) }</a></li>
						</c:forEach>
					</ul>
				</div>
			</div>
	...
	<c:forEach var="allIndexNews" items="${allIndexNewsList }" varStatus="allStatus">
		...
		<c:forEach var="indexNews" items="${allIndexNews }" varStatus="oneStatus">
			<c:if test="${oneStatus.first }">
				...
				<div class="dataHeader">${newsTypeList.get(allStatus.index).typeName }<span class="more"><a href="newsDetail?newsId=${newsTypeList.get(allStatus.index).newsTypeId }">更多...</a></span></div>
				<div class="datas">
				<ul>
			</c:if>
			<li><fmt:formatDate value="${indexNews.publishDate }" type="date" pattern="MM-dd"/>&nbsp;<a href="newsDetail?newsId=${indexNews.newsId }" title="${indexNews.title }">${fn:substring(indexNews.title,0,18) }</a></li>
